/**
 * Turkish InfoTip ([i] tooltip) bodies, keyed by the verbatim English
 * source string. See `../info-tips.ts` for the rationale.
 */

export const TR_INFO_TIPS: Record<string, string> = {
  ...
  ['Literal target of a JavaScript redirect found in an inline script (`window.location = "…"`, `location.href = "…"`, `location.replace("…")`). Followed when "Follow JavaScript redirects" is on.']:
    'Satır içi bir script\'te bulunan JavaScript yönlendirmesinin sabit hedefi (`window.location = "…"`, `location.href = "…"`, `location.replace("…")`). "JavaScript yönlendirmelerini takip et" açıkken takip edilir.',
  ...
  ['Buy Affordable Game Keys | Example Store']: 'Uygun Fiyatlı Oyun Anahtarları | Örnek Mağaza',
  ['Character count of the first H1.']: "İlk H1'in karakter sayısı.",
  ...
  ['HTML attribute name to read.']: 'Okunacak HTML özniteliğinin adı.',
  ...
  ['Indexable / Non-Indexable']: 'İndekslenebilir / İndekslenemez',
  ...
  ['JSON map of `{ term: count }` literal-substring hits from the configured Custom Search terms.']:
    'Yapılandırılan Özel Arama terimlerinden gelen birebir alt dize eşleşmelerinin `{ term: count }` biçimindeki JSON eşlemesi.',
  ...
  ['Number of <img> elements on the page.']: 'Sayfadaki <img> öğelerinin sayısı.',
  ...
  ['On for accessibility / WCAG audits.']: 'Erişilebilirlik / WCAG denetimleri için açık.',
  ...
  ['On for performance-focused audits.']: 'Performans odaklı denetimler için açık.',
  ...
  ['Page that contains the broken link.']: 'Kırık bağlantıyı içeren sayfa.',
  ...
  ['After the crawl, re-fetches a sample of indexable pages with the opposite user agent (mobile when the crawl ran as desktop, desktop otherwise) and compares title, H1, meta description, canonical, robots, word count and link count. Differences feed the \'Mobile / Desktop Mismatch\' issue and the report of the same name.']:
    'Tarama bittikten sonra dizinlenebilir sayfaların bir örneklemini karşı kullanıcı aracısıyla (tarama masaüstü ise mobil, değilse masaüstü) yeniden çeker ve başlık, H1, meta açıklama, canonical, robots, kelime sayısı ve link sayısını karşılaştırır. Farklar \'Mobil / Masaüstü Uyuşmazlığı\' issue\'sunu ve aynı adlı raporu besler.',
  ['How many pages the mobile-parity probe re-fetches, most-linked first. 0 = every indexable HTML page (doubles the crawl\'s traffic for that set).']:
    'Mobil parite sondasının yeniden çektiği sayfa sayısı, en çok link alanlar önce. 0 = dizinlenebilir her HTML sayfa (o küme için tarama trafiğini ikiye katlar).',
  ...
  ['Read the full article →']: 'Makalenin tamamını oku →',
  ...
  ['Sunset over the mountain ridge']: 'Dağ sırtında gün batımı',
  ...
  ['Canonical hops walked after this page (or, on a redirect row, after the redirect\'s final URL) until a page that canonicalises to itself. 0 when the canonical is the page itself or absent.']:
    'Bu sayfadan (yönlendirme satırında, yönlendirmenin son URL\'sinden) itibaren kendi kendine canonical veren bir sayfaya kadar yürünen canonical atlama sayısı. Canonical sayfanın kendisiyse veya yoksa 0.',
  ['Where the canonical chain ends. Empty when the page is its own canonical, or when the chain loops.']:
    'Canonical zincirinin bittiği yer. Sayfa kendi canonical\'ıysa veya zincir döngüye giriyorsa boştur.',
  ...
  ['Total number of <h2> elements on the page.']: 'Sayfadaki toplam <h2> öğesi sayısı.',
  ...
  ['Welcome to Example Store']: 'Örnek Mağazaya Hoş Geldiniz',
  ['What to do when multiple matches exist.']: 'Birden fazla eşleşme olduğunda ne yapılacağı.',
  ...
  ['On — it is a count, so it costs nothing.']: 'Açık — bu bir sayımdır, hiçbir maliyeti yoktur.',
  ...
  ['On for an orphan-page audit.']: 'Orphan sayfa denetimi için açık.',
